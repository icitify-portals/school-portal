...
const sqls = [
  // 1. settlement_accounts (pre-requisite)
  `CREATE TABLE IF NOT EXISTS \`settlement_accounts\` (
    ...
    CONSTRAINT \`settlement_accounts_id\` PRIMARY KEY(\`id\`)
  );`,

  // 2. gateway_subaccounts (pre-requisite)
  `CREATE TABLE IF NOT EXISTS \`gateway_subaccounts\` (
    ...
    CONSTRAINT \`gateway_subaccounts_id\` PRIMARY KEY(\`id\`),
  ...
  // 7. graduate_document_applications
