...
export default function PhotoCapture({ value, onChange, label, applicationId }: PhotoCaptureProps) {
    ...
    return (
        <div className="border-2 border-dashed border-slate-200 rounded-2xl p-4 sm:p-6 bg-slate-50 text-center space-y-4 sm:space-y-6">
            {!isSecureContext && (
                <div className="bg-amber-50 text-amber-800 p-3 rounded-lg text-[10px] sm:text-xs text-left mb-4 font-medium border border-amber-200">
            ...
            <div className="flex flex-col sm:flex-row justify-center items-center gap-2 sm:gap-4 border-b border-slate-200 pb-4">
                ...
                >
                    <Upload className="w-3 h-3 sm:w-4 sm:h-4 mr-2" /> Upload File
                </Button>
                <Button 
                    type="button"
                    variant="ghost" 
                    ...
                    className={cn("w-full sm:w-auto rounded-xl px-4 sm:px-6 py-2 text-[10px] sm:text-xs font-bold uppercase tracking-widest", mode === 'camera' ? "bg-indigo-100 text-indigo-700" : "text-slate-500 hover:bg-slate-100", !isSecureContext && "opacity-50 cursor-not-allowed")}
                >
                    <Camera className="w-3 h-3 sm:w-4 sm:h-4 mr-2" /> Use Camera
                </Button>
            </div>
