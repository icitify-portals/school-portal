...
export default async function ParentDashboard() {
    ...
    return (
        <div className="p-5 md:p-6 max-w-[1600px] w-full mx-auto space-y-6 animate-in fade-in duration-700">
            {/* Header Section */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/50 backdrop-blur-md p-6 rounded-2xl border border-slate-100 shadow-sm">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight mb-1">
                        Parent <span className="text-indigo-600">Dashboard</span>
                    </h1>
                    <p className="text-slate-500 font-medium text-sm">Manage and monitor your children's academic progress across all branches.</p>
                </div>
                <Link 
                    href="/parent/add-child"
                    className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-3 rounded-xl font-bold transition-all shadow-sm w-fit text-sm"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {children?.map((child: any) => (
                    <div 
                        key={child.id}
                        className="group bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col"
                    >
                        {/* Top Accent */}
                        ...
                        <div className="p-6 space-y-5 flex-1">
                            <div className="flex items-start justify-between">
                                <div className="p-3 bg-indigo-50 rounded-xl group-hover:scale-105 transition-transform duration-300">
                ...
                {children?.length === 0 && (
                    <div className="col-span-full py-20 flex flex-col items-center justify-center bg-white/50 rounded-2xl border-2 border-dashed border-slate-200">
