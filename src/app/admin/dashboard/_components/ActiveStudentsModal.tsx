    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col bg-white">
                <DialogHeader>
                    <DialogTitle className="text-xl font-black text-slate-900">Active Students Breakdown</DialogTitle>
                </DialogHeader>
                <div className="overflow-y-auto mt-4">
                    <Table>
                        <TableHeader className="bg-slate-50 sticky top-0">
                            <TableRow>
                                <TableHead className="font-bold text-slate-600">Programme Name</TableHead>
                        ...
                        <TableBody>
                            {breakdown && breakdown.length > 0 ? breakdown.map((item, idx) => (
                                <TableRow key={idx}>
                                    <TableCell className="font-medium text-slate-800">{item.programme}</TableCell>
