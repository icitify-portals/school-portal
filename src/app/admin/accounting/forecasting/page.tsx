                            ...
                            <div className="flex justify-between">
                                <div>
                                    ...
                                    <h4 className="text-xl font-bold text-slate-900">
                                        ₦{(forecast?.projections?.reduce((s: number, p: any) => s + (p.inflow || 0), 0) || 0).toLocaleString()}
                                    </h4>
                                </div>
                        ...
                        <CardContent className="p-6">
                            <div className="flex justify-between">
                                <div>
                                    ...
                                    <h4 className="text-xl font-bold text-slate-900">
                                        ₦{(forecast?.projections?.reduce((s: number, p: any) => s + (p.outflow || 0), 0) || 0).toLocaleString()}
                                    </h4>
                                </div>
                            ...
                            <div className="flex justify-between">
                                <div>
                                    ...
                                    <h4 className="text-xl font-bold text-indigo-900 font-mono">
                                        ₦{(forecast?.projections?.reduce((s: number, p: any) => s + (p.net || 0), 0) || 0).toLocaleString()}
                                    </h4>
                                </div>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {forecast?.projections?.map((p: any, idx: number) => (
                                        <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="py-4 text-right text-emerald-600 font-medium">₦{(p.inflow || 0).toLocaleString()}</td>
                                            <td className="py-4 text-right text-rose-600 font-medium">₦{(p.outflow || 0).toLocaleString()}</td>
                                            <td className={cn(
                                                "py-4 text-right font-black",
                                                (p.net || 0) >= 0 ? "text-slate-900" : "text-rose-700"
                                            )}>
                                                {(p.net || 0) < 0 ? "-" : "+"}₦{Math.abs(p.net || 0).toLocaleString()}
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
