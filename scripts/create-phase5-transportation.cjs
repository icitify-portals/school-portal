  ...
  try {
    ...
    await connection.execute(`
      CREATE TABLE IF NOT EXISTS global_transportation_settings (
        ...
        INDEX idx_settings_campus (campId)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
    `);
    ...
    await connection.execute(`
      CREATE TABLE IF NOT EXISTS transportation_reports (
        ...
        INDEX idx_reports_campus (campId),
    ...
    await connection.execute(`
      CREATE TABLE IF NOT EXISTS transportation_performance_metrics (
        ...
        INDEX idx_metrics_campus (campId),
    ...
    const foreignKeys = [
      'ALTER TABLE multi_campus_coordination ADD CONSTRAINT fk_coordination_source FOREIGN KEY (sourceCampusId) REFERENCES institutional_units(id) ON DELETE CASCADE',
      'ALTER TABLE multi_campus_coordination ADD CONSTRAINT fk_coordination_destination FOREIGN KEY (destinationCampusId) REFERENCES institutional_units(id) ON DELETE CASCADE',
      'ALTER TABLE campus_locations ADD CONSTRAINT fk_locations_campus FOREIGN KEY (campusId) REFERENCES institutional_units(id) ON DELETE CASCADE',
      'ALTER TABLE inter_campus_trips ADD CONSTRAINT fk_inter_coordination FOREIGN KEY (coordinationId) REFERENCES multi_campus_coordination(id) ON DELETE CASCADE',
      'ALTER TABLE inter_campus_trips ADD CONSTRAINT fk_inter_vehicle FOREIGN KEY (vehicleId) REFERENCES vehicles(id) ON DELETE CASCADE',
      'ALTER TABLE inter_campus_trips ADD CONSTRAINT fk_inter_driver FOREIGN KEY (driverId) REFERENCES drivers(id) ON DELETE CASCADE',
      'ALTER TABLE inter_campus_trips ADD CONSTRAINT fk_inter_source FOREIGN KEY (sourceLocationId) REFERENCES campus_locations(id) ON DELETE CASCADE',
      'ALTER TABLE inter_campus_trips ADD CONSTRAINT fk_inter_destination FOREIGN KEY (destinationLocationId) REFERENCES campus_locations(id) ON DELETE CASCADE',
      'ALTER TABLE global_transportation_settings ADD CONSTRAINT fk_settings_campus FOREIGN KEY (campusId) REFERENCES institutional_units(id) ON DELETE SET NULL',
      'ALTER TABLE transportation_api_keys ADD CONSTRAINT fk_api_created_by FOREIGN KEY (createdBy) REFERENCES users(id) ON DELETE CASCADE',
      'ALTER TABLE transportation_audit_logs ADD CONSTRAINT fk_audit_user FOREIGN KEY (userId) REFERENCES users(id) ON DELETE CASCADE',
      'ALTER TABLE transportation_audit_logs ADD CONSTRAINT fk_audit_campus FOREIGN KEY (campusId) REFERENCES institutional_units(id) ON DELETE SET NULL',
      'ALTER TABLE transportation_reports ADD CONSTRAINT fk_reports_generated_by FOREIGN KEY (generatedBy) REFERENCES users(id) ON DELETE CASCADE',
      'ALTER TABLE transportation_reports ADD CONSTRAINT fk_reports_campus FOREIGN KEY (campusId) REFERENCES institutional_units(id) ON DELETE SET NULL',
      'ALTER TABLE emergency_transportation ADD CONSTRAINT fk_emergency_campus FOREIGN KEY (campusId) REFERENCES institutional_units(id) ON DELETE CASCADE',
      'ALTER TABLE emergency_transportation ADD CONSTRAINT fk_emergency_location FOREIGN KEY (locationId) REFERENCES campus_locations(id) ON DELETE SET NULL',
      'ALTER TABLE emergency_transportation ADD CONSTRAINT fk_emergency_requested_by FOREIGN KEY (requestedBy) REFERENCES users(id) ON DELETE CASCADE',
      'ALTER TABLE emergency_transportation ADD CONSTRAINT fk_emergency_approved_by FOREIGN KEY (approvedBy) REFERENCES users(id) ON DELETE SET NULL',
      'ALTER TABLE emergency_transportation ADD CONSTRAINT fk_emergency_vehicle FOREIGN KEY (vehicleId) REFERENCES vehicles(id) ON DELETE SET NULL',
      'ALTER TABLE emergency_transportation ADD CONSTRAINT fk_emergency_driver FOREIGN KEY (driverId) REFERENCES drivers(id) ON DELETE SET NULL',
      'ALTER TABLE transportation_performance_metrics ADD CONSTRAINT fk_metrics_campus FOREIGN KEY (campusId) REFERENCES institutional_units(id) ON DELETE SET NULL',
      'ALTER TABLE transportation_performance_metrics ADD CONSTRAINT fk_metrics_vehicle FOREIGN KEY (vehicleId) REFERENCES vehicles(id) ON DELETE SET NULL',
      'ALTER TABLE transportation_performance_metrics ADD CONSTRAINT fk_metrics_driver FOREIGN KEY (driverId) REFERENCES drivers(id) ON DELETE SET NULL',
      'ALTER TABLE transportation_performance_metrics ADD CONSTRAINT fk_metrics_route FOREIGN KEY (routeId) REFERENCES routes(id) ON DELETE SET NULL',
      'ALTER TABLE transportation_performance_metrics ADD CONSTRAINT fk_metrics_calculated_by FOREIGN KEY (calculatedBy) REFERENCES users(id) ON DELETE CASCADE',
      'ALTER TABLE transportation_integration_logs ADD CONSTRAINT fk_integration_user FOREIGN KEY (userId) REFERENCES users(id) ON DELETE SET NULL'
    ];
...
async function insertPhase5SampleData(connection) {
  ...
  // Get existing data
  const [campuses] = await connection.execute('SELECT id, unitName FROM institutional_units WHERE unitType = "campus" LIMIT 3');
    ...
    if (location.campusId) {
      await connection.execute(`
        INSERT INTO campus_locations (campusId, locationName, locationType, 
          latitude, longitude, address, capacity, facilities, operatingHours, accessibilityFeatures)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        location.campusId, location.locationName, location.locationType,
        location.latitude, location.longitude, location.address, location.capacity,
        location.facilities, location.operatingHours, location.accessibilityFeatures
      ]);
    }
  }
  ...
  for (const coord of coordination) {
    if (coord.sourceCampusId && coord.destinationCampusId) {
      ...
      `, [
        ...
        coord.operatingHours, coord.frequencyMinutes, coord.vehicleCapacity,
        coord.driverRequirements, coord.fareStructure, coord.priorityLevel,
        coord.coordinationRules
      ]);
    }
  }
  ...
  const interCampusTrips = [
    {
      coordinationId: coordination[0]?.id,
      vehicleId: vehicles[0]?.id,
      driverId: drivers[0]?.id,
      tripDate: new Date().toISOString().split('T')[0],
      departureTime: new Date(),
      estimatedArrivalTime: new Date(Date.now() + 45 * 60 * 1000),
      sourceLocationId: campusLocations[0]?.id,
      destinationLocationId: campusLocations[2]?.id,
      passengerCount: 8,
      maxCapacity: 16,
      fareAmount: 100.00,
      status: 'in_progress',
    ...
    {
      coordinationId: coordination[1]?.id,
      ...
      sourceLocationId: campusLocations[2]?.id,
      destinationLocationId: campusLocations[1]?.id,
      passengerCount: 0,
      maxCapacity: 12,
      fareAmount: 120.00,
      status: 'scheduled'
  ...
  for (const trip of interCampusTrips) {
    if (trip.coordinationId && trip.vehicleId && trip.driverId) {
      await connection.execute(`
  ...
  const globalSettings = [
    ...
    {
      settingKey: 'campus_specific_shuttle_frequency',
      ...
      settingType: 'json',
      category: 'operations',
      description: 'Campus-specific shuttle frequency in minutes',
      isGlobal: false,
      campusId: campuses[0]?.id
    }
  ];
    ...
    if (users[0]?.id) {
      await connection.execute(`
        INSERT INTO global_transportation_settings (settingKey, settingValue, 
          settingType, category, description, isGlobal, campusId, lastModifiedBy)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        setting.settingKey, setting.settingValue, setting.settingType,
        setting.category, setting.description, setting.isGlobal,
        setting.campusId, users[0].id
      ]);
    }
  }
  ...
  const emergencyRequests = [
    {
      ...
      severityLevel: 'high',
      campusId: campuses[0]?.id,
      locationId: campusLocations[1]?.id,
    ...
    },
    {
      ...
      campusId: campuses[1]?.id,
      locationId: campusLocations[3]?.id,
  ...
  for (const emergency of emergencyRequests) {
    if (emergency.campusId && emergency.requestedBy) {
      ...
      `, [
        ...
        emergency.locationId, emergency.description, emergency.requestedBy,
        emergency.approvedBy, emergency.vehicleId, emergency.driverId,
        emergency.dispatchTime, emergency.arrivalTime, emergency.completionTime,
        emergency.status, emergency.priorityLevel, emergency.passengersInvolved,
        emergency.specialRequirements, emergency.costEstimate, emergency.actualCost,
        emergency.notes
      ]);
    }
  }
